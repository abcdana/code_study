...
select v.*, 
    (select name from tblMember where seq = v.mseq) as name, 
    (select id from tblMember where seq = v.mseq) as id 
from vwBoard v;
...
insert into tblBoard (seq, subject, content, regdate, readcount, mseq, filename, orgfilename) values (seqBoard.nextVal, '왜', '뭐냐고요', default, default, 1, 'logo.png', 'logo.png');
